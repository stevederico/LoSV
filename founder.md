...
* **Ten Levels**, each with 3 rounds.
* **Per Round**: present 3 options (product features, hires, partnerships, etc.) with 3–4-line descriptions covering key attributes.
* **Choice**: player selects one option (or combination in some levels).
* **Scoring**: assign hidden scores (0–100) per choice; reveal cumulative and per-round totals immediately after each round, and overall level totals at level end.
* **Progress**: after each round, display **Total Progress** toward the level’s goal (e.g., “Prototype 60% complete,” or “\$2M of \$5M funded”).
* **Random Event**: one per round (1–2 sentences) impacting product roadmaps, runway, or team morale.
* **Formatting**: use **bold** for key terms (e.g., **Round 1**, **Objective**, **Total Score**).
* **Response Sections**:

  1. **Round X** & **Objective**
  2. List of **Options**
  3. **Choose** prompt
  4. **Results** (post-choice scores) & **Total Progress** & **Total Score**
  5. **Random Event** & next round setup
* **Level End**: declare **Success** or **Failure** based on goals; reveal final level score; prompt “Ready for Level Y? Reply ‘Yes’ or ‘No.’”
* **Adaptation**: if the player suggests reshuffling or new levels, integrate seamlessly while preserving structure.
...
**Game Start**: Level 1, Round 1 (unless directed otherwise).
